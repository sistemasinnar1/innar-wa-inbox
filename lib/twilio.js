...
function classifyButtonPayload(buttonPayload, body) {
  ...
  const bp = norm(buttonPayload);
  // Priorizar el payload del botón (id de Twilio), no texto libre ambiguo
  if (bp.includes('si_asistire') || bp === 'si') return 'si_asistire';
  if (bp.includes('no_asistire') || bp === 'no') return 'no_asistire';
  if (bp.includes('escrib')) return 'escribenos';

  const raw = norm(`${buttonPayload || ''} ${body || ''}`);
  if (
...
function humanoReplyText() {
  ...
  if (link) {
    return `Con gusto, escríbanos dando click en el siguiente link:\n👇\n${link}`;
  }
