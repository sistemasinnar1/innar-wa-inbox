...
/** Color de evento en Google Calendar cuando cancela (Flamingo — rosado). */
const EVENT_COLOR_CANCELADO = '4';
/** Basil: verde solo cuando el recordatorio de ESA cita ya se envió. */
const EVENT_COLOR_ENVIADO = '10';
/** Graphite: sin verde mientras el mensaje no salga. */
const EVENT_COLOR_PENDIENTE = '8';
const EVENT_COLORS_VERDE = new Set(['2', '10']);
...
function mapApiEvent(ev, nombreHoja, dateYmd) {
  ...
  return {
    ...
    attendance,
    color_id: ev.colorId ? String(ev.colorId) : ''
  };
}

function eventoFuturoSinEnviar(ev, now = Date.now()) {
  if (!ev || ev.enviado || ev._enviado) return false;
  if (String(ev.attendance || '') === 'cancelado') return false;
  const ms = Date.parse(ev.start_iso || '');
  if (Number.isNaN(ms)) return false;
  return ms > now;
}

function colorVerdePendiente(ev) {
  if (!eventoFuturoSinEnviar(ev)) return false;
  const color = String(ev.color_id || '');
  return !color || EVENT_COLORS_VERDE.has(color);
}

async function setEventColor({ calendarKey, eventId, colorId }) {
  const calendarios = loadCalendarios();
  const calendarId = calendarios[calendarKey];
  if (!calendarId || !eventId || !colorId) return null;
  const auth = getAuth();
  const calendar = google.calendar({ version: 'v3', auth });
...
async function createEvent({
  ...
  const res = await calendar.events.insert({
    ...
    requestBody: {
      ...
      // Sin verde hasta que salga el recordatorio de esta cita
      colorId: EVENT_COLOR_PENDIENTE,
 ...
 * y solo si es cancelado se colorea el evento (Flamingo / rosado).
 */
async function updateEventAttendance({ calendarKey, eventId, status }) {
  const calendarios = loadCalendarios();
  const calendarId = calendarios[calendarKey];
  if (!calendarId) {
    const err = new Error(`Calendario desconocido: ${calendarKey}`);
  ...
  // Solo «No asistiré» colorea el evento (Flamingo / rosado). Confirmar no cambia color.
  if (value === 'cancelado') requestBody.colorId = EVENT_COLOR_CANCELADO;
...
/**
 * Aplica confirmado/cancelado + color SOLO a un event_id concreto.
 */
async function applyAttendanceToEvent({ calendarKey, eventId, status }) {
  if (!googleConfigured()) return { updated: 0, items: [] };
  if (!calendarKey || !eventId) return { updated: 0, items: [] };
  const updated = await updateEventAttendance({ calendarKey, eventId, status });
...
module.exports = {
  ...
  EVENT_COLOR_ENVIADO,
  EVENT_COLOR_PENDIENTE,
  eventoFuturoSinEnviar,
  colorVerdePendiente,
  setEventColor,
  telefonoEstado,
  tipoYProfesional,
