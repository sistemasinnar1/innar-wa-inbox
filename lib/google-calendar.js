...
/**
 * Parsea start de Google Calendar siempre como hora Colombia.
 * Si dateTime viene sin offset (ej. 2026-09-21T10:00:00), Node en UTC
 * lo tomaría como UTC y la hora saldría corrida −5 h.
 */
function parseGoogleDateTime(startObj) {
  if (!startObj) return null;
  if (startObj.dateTime) {
    let raw = String(startObj.dateTime).trim();
    if (/[zZ]$|[+-]\d{2}:?\d{2}$/.test(raw)) {
      return new Date(raw);
    }
    // Hora “local” del calendario → forzar Colombia (UTC-5)
  ...
  if (startObj.date) {
    // Todo el día: mediodía Colombia para no cruzar de fecha
