...
async function sendRemindersForEvents(events, { emit } = {}) {
  ...
  for (const ev of events) {
    ...
    try {
      ...
      if (ev.event_id && ev.calendar_key && cal.googleConfigured()) {
        try {
          await cal.setEventColor({
            calendarKey: ev.calendar_key,
            eventId: ev.event_id,
            colorId: cal.EVENT_COLOR_ENVIADO
          });
        } catch (colorErr) {
          console.warn('[Calendar] No se pudo marcar verde el evento enviado:', colorErr.message);
        }
      }
...
/** Event IDs que ya tienen recordatorio WhatsApp enviado. */
...
/** Anota `enviado: true` en cada evento que ya tuvo recordatorio. */
async function annotateSentOnListed(listed) {
  ...
  listed.calendars.forEach((c) => {
    (c.events || []).forEach((ev) => {
      if (ev && ev.event_id && sent.has(String(ev.event_id))) {
        ev.enviado = true;
        ev._enviado = true;
      } else if (ev) {
        ev.enviado = false;
  ...
  if (Array.isArray(listed.flat)) {
    listed.flat.forEach((ev) => {
      if (ev && ev.event_id && sent.has(String(ev.event_id))) {
        ev.enviado = true;
        ev._enviado = true;
      } else if (ev) {
...
/** Quita el verde de Google Calendar en citas futuras cuyo recordatorio aún no salió. */
async function syncUnsentFutureColors(listed) {
  if (!listed || !cal.googleConfigured()) return listed;
  const all = listed.flat || listed.calendars.flatMap((c) => c.events || []);
  for (const ev of all) {
    if (!cal.colorVerdePendiente(ev) || !ev.event_id || !ev.calendar_key) continue;
    try {
      await cal.setEventColor({
        calendarKey: ev.calendar_key,
        eventId: ev.event_id,
        colorId: cal.EVENT_COLOR_PENDIENTE
      });
      ev.color_id = cal.EVENT_COLOR_PENDIENTE;
    } catch (err) {
      console.warn('[Calendar] No se pudo quitar el verde:', ev.event_id, err.message);
    }
  }
  return listed;
}
